...
> **Hot reload caveat**: a required library reloads on edit only if it is written in **CommonJS** style (`module.exports = …`). A library using `import` / `export` is cached a second time inside Node's ESM loader, which `require.cache` eviction cannot reach, so edits to it stay invisible until you restart the server. Write docroot libraries as CJS to keep the edit-refresh loop (`export const x` → `module.exports.x`), or restart after editing an ESM one. Details and the pending fix options: [docs/draft_reload.md](docs/draft_reload.md).
