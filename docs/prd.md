...
- The server's own source file is never served (404) even when the document root is the package directory — matched case-insensitively on Windows (covering case variants and 8.3 short names);
...
- 404 handling: missing files and rejected path traversal both return 404, indistinguishable between "does not exist" and "denied";
...
- `_SESSION` — session object: based on **signed cookie + timestamp**, no server-side storage; cookie fixed at `Path=/; HttpOnly; SameSite=Lax`, browser-session cookie (no Max-Age); sliding timeout (default 30 minutes, re-signed and refreshed on every response); the signing key is derived from a local machine fingerprint (hostname + username + home directory + NIC MACs + the document root's realpath, SHA256), so nothing is written to disk yet it stays stable on the same machine, differs across machines, and **differs across sites on the same machine**; the whole cookie over 4KB → 500 (explicit failure beats silent browser drop); values must be JSON-serializable; data is visible to the client (tamper-proof, not confidential). **Strength boundary**: every fingerprint component is enumerable by any local user of the same machine, so "tamper-proof" holds against the client side only — another local user on a shared host can forge cookies; not designed for multi-user-host isolation;
...
- `RESP.status(code)` / `RESP.header(name, value)` — set status code / response header (status not an integer in 100–999 → 500); **hop-by-hop header names** (`Transfer-Encoding`, `Connection`, `Trailer`, `Upgrade`, …) are framework-owned: template attempts are dropped with a stderr warning (they would clash with the buffered-body Content-Length model);
