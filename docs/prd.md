...
- **A loopback bind is not an access control**: any page the developer's browser visits can point a hostname it controls at `127.0.0.1` (DNS rebinding) and then talk to this server as same-origin — reading responses included. The server therefore checks the `Host` header and answers only to loopback names, `*.localhost`, literal IP addresses (IPv4 and IPv6, bracketed or bare) and its own bind address; anything else gets a 403 naming `--allowed-hosts`. This is the one attack class the "local / trusted environment" positioning does not cover on its own, because the attacker is remote while the victim's own browser makes the request.
...
- Requests for non-.eta files go through static file serving: per a **built-in extension whitelist** (web .html/.htm, text .txt, .css, .js, data .json, common images, .pdf, common archives — full table in the spec) the matching Content-Type and raw bytes are returned; **anything outside the whitelist is 404**, denied by default (fail-closed); whitelisted existing files accept GET/HEAD only — other verbs get 405 Method Not Allowed (with an `Allow: GET, HEAD` header), and the same GET/HEAD rule covers the `index.html` / `index.htm` directory candidates (a POST to `/sub/` answers 405 exactly like a POST to `/sub/index.html`); .eta scripts — `index.eta` included — are rendered for all verbs, with `REQUEST_METHOD` passed through faithfully;
...
- Relative path traversal outside the document root is prevented; **filesystem-level escapes are rejected too**: symlinks / junctions inside root pointing outside fail the realpath containment check and 404; Windows reserved device names (NUL/CON…), NTFS alternate data streams (`$DATA`) 404; trailing dots/spaces are normalized per Win32 file-open semantics; duplicate slashes collapse with a 308 (`//a` → `/a`), and because `\` is equivalent to `/` in an http(s) URL (WHATWG), a target carrying a raw backslash is normalized the same way (`/\host/x.eta` → 308 `/host/x.eta`) instead of being parsed as a host and routed elsewhere — a literal backslash in a filename stays reachable as `%5C`;
...
- The server's own source file is never served (404) even when the document root is the package directory — matched case-insensitively on every platform, with paths canonicalized through `fs.realpathSync.native` so that **win32 8.3 short names** (`ETA-SE~1.JS`) resolve to the real long name instead of sneaking past. The same canonicalization protects the hidden-path convention: `/NODE_M~1/`, `/LIB~1/` and friends are 404 exactly like their long names;
...
- Change detection: the template engine cache is disabled (`cache: false`); every request reads from disk and recompiles — edits take effect immediately; responses ship `Cache-Control: no-store` (both rendered pages and static assets) so the browser cannot serve a heuristically cached copy of a file you just edited — templates wanting real caching set the header themselves; files loaded via `require()` under the document root also hot-reload on edit (module-cache entries invalidated by mtime; `node_modules` excluded; invalidation covers the edited file itself — see the spec for the shallow-invalidation caveat);
...
- `_GET` / `_POST` — GET and POST parameter dicts separately; `_POST` parses `application/x-www-form-urlencoded` (bounded at 4096 pairs, over-limit returns 413) and `multipart/form-data` (fields only; file uploads go into `_FILES`); JSON request bodies go into `_JSON`;
- `_ENV` — environment variables snapshot (null-prototype dict, equivalent to PHP `$_ENV`);
- `_FILES` — uploaded files from `multipart/form-data` requests, following PHP's `$_FILES` shape (`name` / `type` / `size` / `tmp_name` / `error` per field); **file uploads are opt-in**: by default (no `--allow-uploads`) nothing is written to disk — file parts still arrive in `_FILES` with `error = 8` (`UPLOAD_ERR_EXTENSION`, PHP's "upload stopped by policy" code) and an empty `tmp_name`, so a template can tell "uploads disabled" from "no file selected", while regular multipart fields keep parsing into `_POST`; once enabled, file contents are stored in temp files that are automatically cleaned up after the response; repeated `name="f[]"` uploads produce the array-of-columns structure PHP uses; uploads are bounded by field/file/header/size limits and over-limit requests receive 413 — the part-count limit is enforced **while** the body is split, so a boundary chosen to be tiny (an attacker-controlled `Content-Type` detail) cannot amplify memory past the limit before it fires;
...
- `_SESSION` — session object: based on **signed cookie + timestamp**, no server-side storage; cookie fixed at `Path=/; HttpOnly; SameSite=Lax` (plus `Secure` when the client used https, which is only knowable behind `--behind-proxy` via `X-Forwarded-Proto`), browser-session cookie (no Max-Age); sliding timeout (default 30 minutes, configurable in minutes via `--session-ttl <m>` / the `sessionTtl` library option, re-signed and refreshed on every response); reassigning `_SESSION` wholesale (`_SESSION = {}`, `null`, `undefined` or `false`) clears the session — the reassignment is read back after rendering, and a cleared session emits the `Max-Age=0` cookie-clearing line, but **only for a cookie that verified**: a value this server cannot verify (another instance's, tampered, expired) is left untouched rather than deleted; the signing key is derived from a random per-user secret persisted at `~/.eta-server/session-secret` (mode 0600) mixed with the document root's realpath (HMAC), so it stays stable across restarts and NIC changes yet **differs across sites on the same machine** (a diskless machine-fingerprint fallback applies when the home directory is not writable), and `--secret` / `ETA_SERVER_SECRET` replaces the whole derivation with a value you choose (see the CLI section); the whole cookie over 4KB → 500 (explicit failure beats silent browser drop); values must be JSON-serializable — a BigInt / circular value gives a 500 that names the constraint (and a `.404.eta` page degrades to the built-in 404 instead of escalating); data is visible to the client (tamper-proof, not confidential). **Strength boundary**: "tamper-proof" holds against the client side; on a shared host, another local user who can read the secret file can forge cookies; not designed for multi-user-host isolation. **Cookie-name collision**: the cookie name is the constant `etasess` and cookies are not port-scoped, so two servers with different roots on `localhost` compete for one cookie slot. A site never deletes a cookie it cannot verify, so browsing site B no longer logs you out of site A; but if both sites write sessions, each response overwrites the other's value and the two sessions keep resetting each other. Give both instances the same `--secret` and they share one session instead;
...
- `RESP.setcookie(name, value, opts)` — set a cookie, values percent-encoded by default; non-numeric `maxage` is ignored (never emits `Max-Age=NaN`); **entries colliding with the session cookie name (`ETASESSION`) are dropped and never sent** (the session mechanism owns that name; a warning goes to stderr);
...
- `RESP.write(str)` / `echo(str)` — output text from inside a code block, like PHP `echo`; the output interleaves correctly with surrounding template text and `<%= %>` interpolation, and is short-circuited by `writeraw()` and `json()` like ordinary template text;
...
- `--allowed-hosts <list>`: comma-separated extra `Host` names to accept, on top of the always-allowed loopback names / `*.localhost` / literal IPs / bind address; `all` disables the check entirely (with a startup warning). HTTP mode only;
- `--secret <value>`: set the session signing key yourself instead of letting the server derive one (env `ETA_SERVER_SECRET`, which the flag overrides; a missing or blank value is an error, not a silent fallback). An explicit secret replaces **both** automatic inputs — the persisted per-user secret and the per-root mixing — so the same value always yields the same key: reproducible in containers / CI with no writable home, rotatable on demand, and **instances sharing a secret accept each other's session cookies**, which is the way out of the cookie-name collision described under `_SESSION`. The trade-off is printed at startup: per-site isolation is off, so every document root started with that secret shares one session namespace. Prefer the env var over the flag where other users can read the process list. HTTP mode only;
- `--session-ttl <minutes>`: sliding session timeout length in minutes (default: 30; positive numbers, fractions allowed). Invalid values are rejected up front — exit 1 from the CLI, a rejected Promise from `startServer` — never a silent fallback to the default. Semantics stay idle-based sliding: the value is the time after the *last* request, re-signed on every response. HTTP mode only;
- `--behind-proxy`: declare that a reverse proxy owns this port — skips the Host check and **trusts** `X-Forwarded-For` / `X-Real-IP` (→ `_SERVER.REMOTE_ADDR` and the access-log client field), `X-Forwarded-Proto` (→ `_SERVER.REQUEST_SCHEME`) and `X-Forwarded-Host` (→ `_SERVER.SERVER_NAME`). Without the flag those headers are ignored, since a DNS-rebound page is same-origin and could forge them. Even with it the values are **validated**: a forwarded client address must be an IP literal (an appended source port is tolerated) or the real peer is used instead, and a forwarded host must match the RFC 1123 host grammar or the `Host` / bind address is used — the flag says a proxy owns the port, which does not make the header contents true. Combining it with `--allowed-hosts` keeps the allowlist enforced (naming your domains is stronger than trusting the proxy). Keep the bind address private (`-H 127.0.0.1`) when using it. HTTP mode only;
- `--allow-uploads`: allow `multipart/form-data` **file uploads** (the `_FILES` channel). Default is **off**: multipart form fields still parse into `_POST`, but file parts are never written to disk — they arrive in `_FILES` with `error = 8` (`UPLOAD_ERR_EXTENSION`) and an empty `tmp_name`. The counterpart of PHP's `file_uploads` directive with a safer default (PHP ships it on; a dev server that never asked for uploads should not write files to disk for anyone who can reach the port). The library API equivalent is `options.allowUploads`. All parse-time guards stay active when disabled. HTTP mode only;
- `-h / --help`: help.
