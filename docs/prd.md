...
- `RESP.status(code)` / `RESP.header(name, value)` — set status code / response header (status not an integer in 100–999 → 500);
...
- `RESP.setcookie(name, value, opts)` — set a cookie, values percent-encoded by default; **entries colliding with the session cookie name (`etasess`) are dropped and never sent** (the session mechanism owns that name; a warning goes to stderr);
