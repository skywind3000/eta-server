...
- **Hidden-path convention** (server-side libraries and configs stay private): any path segment starting with `_` or `.`, and `node_modules` directories, is never served — neither as a template nor as a static file nor as a directory index (404, indistinguishable from "does not exist"). Put decision #9's library files and config files behind such names (`_lib/util.ts`, `_config.json`); plain assets (`style.css`, `app.js`) stay public. Note a DIRECTORY whose name contains `.eta` is served normally (the script/PATH_INFO split applies to files only);
