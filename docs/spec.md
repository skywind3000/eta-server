...
Single file `eta-server.js` (~1700 lines), sole runtime dependency `eta`.
...
HTTP mode has 78 checks (symlink-dependent probes self-SKIP on systems without file-symlink privilege — win32 needs elevation / developer mode; 8.3 probes self-SKIP on volumes with short-name creation disabled; junction-based probes stay unconditional):
...
- **session robustness (decision #22)**: a non-serializable `_SESSION` value (BigInt, circular) gives a 500 naming the constraint on a normal page and degrades to the built-in 404 from `.404.eta` (a sixth condition on that fixture); `_SESSION` is null-prototype on the fresh (no-cookie) request as well as the restored one;
...
- **reverse proxy (decision #20 amendment)**: `X-Forwarded-For` / `-Proto` / `-Host` are ignored by default (a rebound page could forge them) and trusted under `--behind-proxy`, which also skips the Host check, reports the forwarded client in the access log and drops a bogus scheme value; `--allowed-hosts` alongside `--behind-proxy` still outranks it;
- **Host allowlist (decision #20)**: a foreign Host gets 403 naming `--allowed-hosts` and one stderr line, while loopback names / `*.localhost` / bare IPv4 / `[::1]` keep working; `--allowed-hosts a,b` admits exactly those names and `--allowed-hosts all` admits everything (two in-process instances);
