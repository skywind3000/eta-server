...
- **index fallback candidates verified one by one** (review follow-up): the directory itself passes containment, but `index.eta` / `index.html` / `index.htm` inside it could be links pointing outside root, so each candidate goes through `realInside`; an escape means 404 (fail-closed, no fallback to the next candidate);
