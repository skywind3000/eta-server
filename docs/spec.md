...
| `_SESSION` | object | signed-cookie session (decision #5); whole cookie over 4KB → 500 (decision #13); wholesale reassignment (`_SESSION = {}` / `null` / `undefined` / `false`) clears the session, read back after rendering (decisions #17/#20); null-prototype like every other dict (decision #20) |
...
HTTP mode has 71 checks (symlink-dependent probes self-SKIP on systems without file-symlink privilege — win32 needs elevation / developer mode; 8.3 probes self-SKIP on volumes with short-name creation disabled; junction-based probes stay unconditional):
