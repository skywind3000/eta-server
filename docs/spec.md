...
`_SERVER` keys: `REQUEST_METHOD`, `QUERY_STRING` (raw request-line slice, encoded original text), `REQUEST_URI` (req.url original), `SCRIPT_NAME`, `PATH_INFO`, `SCRIPT_FILENAME`, `SCRIPT_DIRNAME`, `DOCUMENT_ROOT`, `REMOTE_ADDR`, `CONTENT_TYPE`, `CONTENT_LENGTH`, `SERVER_NAME` (hostname from the allowlisted Host header, bind address as fallback — decision #20), `SERVER_PORT`, `REQUEST_SCHEME` (always `http`), `SERVER_PROTOCOL` (`HTTP/` + req.httpVersion), `REQUEST_TIME` / `REQUEST_TIME_FLOAT` (request start instant, set in both HTTP and CLI modes), plus `HTTP_*` request headers (uppercased, `-`→`_`).
...
HTTP mode has 69 checks (symlink-dependent probes self-SKIP on systems without file-symlink privilege — win32 needs elevation / developer mode; 8.3 probes self-SKIP on volumes with short-name creation disabled; junction-based probes stay unconditional):
